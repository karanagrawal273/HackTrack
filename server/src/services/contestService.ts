...
const PLATFORM_MAP: { [key: string]: IContest['platform'] } = {
 ...
 */
export const fetchAndSaveContests = async (): Promise<number> => {
  ...
  try {
    ...
    if (finishedContests.length > 0) {
      ...
      const calendarDeleteResult = await CalendarEvent.deleteMany({
          contest: { $in: finishedContestIds }
      });
      
      // b. Delete the Contest records themselves
      const contestDeleteResult = await Contest.deleteMany({
    ...
    for (const contestData of contests) {
      ...
      const result = await Contest.findOneAndUpdate(
        { contestId: contestToSave.contestId }, // Query
